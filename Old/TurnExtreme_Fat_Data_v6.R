...
library("wmtsa")
library("reshape")
source("FatTails_Functions_v6.R")
...
save(All_Params, Lake_Params3, file="All_Params_TurnExtreme_Fat_Data_v6.RData")
...
# All_Params[All_Params[,"N"]<400,][154,]
# 
...
sAP <- All_Params[,c("Type", "fitBy", "Variable", "Duration", "N", "mu_0", "sig_0", "sh_0", "Level1", "Level2", "mean", "sd", "Level1_time", "Level2_time", "Level1_normTime", "Level2_normTime")]
# sAP[is.nan(sAP)] <- NA
# sAP <- Inf2NA(sAP)
...
# lvl2Diff <- sAP2[,"Level2_normTime"] - sAP2[,"Level2_time"]
# colorCode <- c("black", "red")[c(lvl2Diff>0) + 1]
# plot(sAP2[,"sh_0"], log10(abs(lvl2Diff)), col=colorCode)

dev.new(height=5, width=6)
par(mfrow=c(2,2), mar=c(3,3,0.5,0.5), oma=c(1,2,1.5,0), ps=10, cex=1)
plot(sAP[,"sh_0"], log10(sAP[,"Level1_normTime"]))
mtext("Return Time (log10(yrs)) \n [mean+2*sd]", side=2, line=2.5)
mtext("Predicted by \n Normal Distribution", side=3, font=2)
plot(sAP[,"sh_0"], log10(sAP[,"Level1_time"]))
mtext("Predicted by \n GEV Distribution", side=3, font=2)
plot(sAP[,"sh_0"], log10(sAP[,"Level2_normTime"]))
mtext("Return Time (log10(yrs)) \n [record+20%]", side=2, line=2.5)
plot(sAP[,"sh_0"], log10(sAP[,"Level2_time"]))
mtext(expression(Value~~of~~xi~~from~~GEV), outer=TRUE, line=-0.5, side=1)


# dev.new(height=5, width=6)
# par(mfrow=c(2,2), mar=c(3,3,0.5,0.5), oma=c(1,2,1.5,0), ps=10, cex=1)
# sAP_cv <- sAP[,"sd"]/sAP[,"mean"]
# plot(sAP_cv, log10(sAP[,"Level1_normTime"]))
# mtext("Return Time (log10(yrs)) \n [mean+2*sd]", side=2, line=2.5)
# mtext("Predicted by \n Normal Distribution", side=3, font=2)
# plot(sAP_cv, log10(sAP[,"Level1_time"]))
# mtext("Predicted by \n GEV Distribution", side=3, font=2)
# plot(sAP_cv, log10(sAP[,"Level2_normTime"]))
# mtext("Return Time (log10(yrs)) \n [record+20%]", side=2, line=2.5)
# plot(sAP_cv, log10(sAP[,"Level2_time"]))
# mtext(expression(sigma/mu), outer=TRUE, line=-0.5, side=1)


# dev.new(height=3.5, width=4)
# par(mar=c(3,3,0.5,0.5), ps=10, cex=1)
# plot(sAP[,"sh_0"], sAP[,"mean"]/(sAP[,"Level2"]/1.2))
# mtext(expression(xi), side=1, line=2); mtext("mean/record", side=2, line=2)

dev.new(width=3.5, height=6)
par(mfrow=c(2,1), mar=c(3,4,0.5,0.5), ps=10, cex=1)
boxplot(log10(Level2_time)~Type, data=sAP, ylab="")
mtext("Return Time (log10(yrs)) \n [record+20%]", side=2, line=2)
