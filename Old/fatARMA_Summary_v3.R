...
source("TonySuggestions/tonyARMA_short_v6.R") #also loads GenSA and DEoptim packages
...
summary(lm(sigInf~Type, data=final)) # but the type of variable doesn't predict the stationary varince very well ...
...
par(mfrow=c(2,2), mar=c(3.5,3.5,0.5, 0.5), ps=10, mgp=c(1.5,0.5,0), tcl=-0.4, family="Times")
boxplot(log10(Level2_time)~Type, data=final, outline=FALSE, ylab=("log10(Years) until 1.2*max(Obs)"))

#How often are you suprised if you do know where you are? (Xi from ARMA residuals) Answer:
boxplot(log10(Level2_time_residual)~Type, data=final, outline=FALSE, ylab=("log10(Years) until 1.2*max(ARMA Residual)"))
...
plot(xfrm_lvl2Res, xfrm_lvl2TS, xlab=("1/log10(Time until 1.2*max(ARMA Residual))"), ylab=("1/log10(Time until 1.2*max(Obs))"))
...
plot(tsDens, main="", xlab="log10(Years Waiting until Record Broken by 20%)", ylim=range(c(tsDens$y, residDens$y)), lwd=2, zero.line=FALSE)
...
png("FatFigures_v3/lambda_order_N.png", width=3.4, height=3.4, units="in", res=300, bg=myWhite)
...
png("FatFigures_v3/lambda_variance_hist.png", width=6, height=3.4, units="in", res=300, bg=myWhite)
...
#Figure 2
# dev.new(width=6, height=3.4)
png("FatFigures_v3/variance_lambda.png", width=6, height=3.4, units="in", res=300, bg=myWhite)
library("gridExtra")
library("ggplot2")
...
# dev.new(); plot((final[final[,"P"]==1&final[,"Q"]==0,"Lambda"]), log10(final[final[,"P"]==1&final[,"Q"]==0,"InfE"])) #rest assured, Lambda is perfectly related to sigInf/sigE for AR model
...
# dev.new(width=3.4, height=3.4)
png("FatFigures_v3/Xi_InfE.png", width=3.4, height=3.4, units="in", res=300, bg=myWhite)
dscat(log10(final[gg_sh_logic,"InfE"]), final[gg_sh_logic,"sh_0"], mar=c(2.5,2.5,0,0), cex=1, ps=9, family="Times", mgp=c(3, 0.5, 0), tcl=-0.35)
abline(lm(sh_0~log10(InfE), data=final[gg_sh_logic,]), lty="dashed", lwd=2)
...
png("FatFigures_v3/Xi_Lambda.png", width=3.4, height=3.4, units="in", res=300, bg=myWhite)
dscat(sqrt(final[gg_sh_logic,"Lambda"]), final[gg_sh_logic,"sh_0"], mar=c(2.5,2.5,0,0), cex=1, ps=9, family="Times", mgp=c(3, 0.5, 0), tcl=-0.35)
abline(lm(sh_0~sqrt(Lambda), data=final[gg_sh_logic,]), lty="dashed", lwd=2)
mtext(bquote(xi), side=2, line=1.35, cex=1)
mtext(bquote(sqrt(.(lNota2))), side=1, line=1.5)
dev.off()

# New _v2
png("FatFigures_v3/Xi_sigE.png", width=3.4, height=3.4, units="in", res=300, bg=myWhite)
...
# New _v3
png("FatFigures_v3/xiTs_xiResid.png", width=3.4, height=3.4, units="in", res=300, bg=myWhite)
...
png("FatFigures_v3/Xi_sigE_IncludePhys_sqrt.png", width=3.4, height=3.4, units="in", res=300, bg=myWhite)
dscat(sqrt(final[gg_sh_logic,"sigE"]), final[gg_sh_logic,"sh_0"], mar=c(2.5,2.5,0,0), cex=1, ps=9, family="Times", mgp=c(3, 0.5, 0), tcl=-0.35)
abline(lm(sh_0~sqrt(sigE), data=final[gg_sh_logic,]), lty="dashed", lwd=2)
mtext(bquote(xi[Time~Series]), side=2, line=1.35, cex=1)
...
# ==========================
png("FatFigures_v3/Box_ReturnShape.png", width=7, height=3.4, units="in", res=300, bg=myWhite)
...
# ==================================================================
png("FatFigures_v3/ScatterReturns.png", width=6, height=2, bg=myWhite, units="in", res=300)
...
# ========================
png("FatFigures_v3/ScatterReturns_2blank.png", width=6, height=2, bg=myWhite, units="in", res=300)
...
# ========================
png("FatFigures_v3/ScatterReturns_allBlank.png", width=6, height=2, bg=myWhite, units="in", res=300)
...
png("FatFigures_v3/Returns.png", width=7, height=3.4, units="in", res=300, bg=myWhite)
...
png("FatFigures_v3/Returns_1.png", width=3.4, height=3.4, units="in", res=300, bg=myWhite)
...
png("FatFigures_v3/fatTimeSeries.png", width=5, height=7, bg=myWhite, units="in", res=300)
