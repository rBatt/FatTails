...
# EpiMean2 <- function(x){ #this version will return NA if there is no zmix, and the only readings for a parameter are at depths greater than min(depth).  E.g., depths for 0, 0.5, 1m.  Chlor has value at 0, 0.5, 1, function will return the 0m value.  pH might have values at 0.5 and 1, but function will return NA, because it is only looking for values at 0m, b/c 0m is shallowest depth on that day.
# 	stopifnot(all(is.element(c("depth","Zmix"), names(x))))
# 	xnames <- names(x)[!is.element(names(x), c("lakeid", "year4", "daynum", "sampledate"))]
# 	x2 <- x[,xnames]
# 
# 	if(all(is.na(x2[,"Zmix"]))){ #if zmix is unknown, just return the shallowest values
# 		if(is.null(nrow(x2))){return(x2)}
# 		shall <- which.min(x2[,"depth"])
# 		x3 <- colMeans(x2[shall,], na.rm=TRUE) #this function is actually broken b/c x3 is never returned. Instead, just the mean of all of the epi values will be returned in the last line of the function
# 	}
# 	
# 	zd <- x2[,"Zmix"] - x2[,"depth"]
...
# = Ions =
# ========
Ions000 <- read.csv("Ions_NrtnSrtn.csv") #where are there so many rows with no data?
...
Ions_Not_All_NA_Ind <- which(!apply(Ions0[,c("cl", "so4", "ca", "mg", "na", "k", "fe", "mn", "cond")], MARGIN=1, FUN=allna))
Ions0 <- Ions0[Ions_Not_All_NA_Ind,]
Ions0 <- aggregate(Ions0[,c("Zmix", "cl", "so4", "ca", "mg", "na", "k", "fe", "mn", "cond")], by=Ions0[,rev(c("lakeid", "year4", "daynum", "sampledate", "depth"))], FUN=mean, na.rm=TRUE)
...
# = Chemical Limnology =
# ======================
Chem000 <- read.csv("Chem_NrtnSrtn.csv")
...
save(Data_X, file="OrganizedFatData_Read_Fat_Data_v1.RData")
