...
# =================================
# = Fill in missing years w/ NA's =
# =================================
fillMiss <- function(x){
	hasDim <- !is.null(dim(x))
	hasNames <- all(c("year4","Data") %in% colnames(x))
	if(hasDim&hasNames){
		x1 <- as.integer(x[,"year4"])
		x[,"year4"] <- x1
		xo <- as.integer(do.call(":", sapply(range(x1), list)))
		
		merge(x[,c("year4","Data")], data.frame(year4=xo), all=TRUE)
		
	}else{
		stop("must supply matrix or data.frame w/ year4 and Data columns")
	}
}
