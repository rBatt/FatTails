...
png("/Users/Battrd/Documents/School&Work/WiscResearch/FatTails/Figures/dscat_xiTS_lambda.png", res=150, units="in", height=3.5, width=3.5)
# dev.new(height=3.5, width=3.5)
...
png("/Users/Battrd/Documents/School&Work/WiscResearch/FatTails/Figures/dscat_xiTS_lambda.png", res=150, units="in", height=3.5, width=3.5)
# dev.new(height=3.5, width=3.5)
dscat(z[,"p"]+z[,"q"], z[,"xi2"], mar=c(2.5,2.5,0,0), cex=1, ps=9, family="Times", mgp=c(3, 0.5, 0), tcl=-0.35)
# abline(lm(xi~xi.resid, data=z[,]), lty="dashed", lwd=1)
mtext(bquote(xi[Time~Series]), side=2, line=1.35, cex=1)
mtext(bquote(p+q), side=1, line=1.5)
dev.off()
