...
load("FatTails_rawData/OrganizedFatData_Read_Fat_Data_v2.RData")
library("wmtsa")
library("reshape")
...
Met_lvl <- lvl(x=Data_X$Met, variables=Met_vars, fitby="location", lvl2Thresh=setThresh)
...
Ice_lvl <- lvl(x=Ice_Ext, variables=Ice_vars, fitby="Region", lvl2Thresh=setThresh)
Ice <- merge(Ice_Params, Ice_lvl[1,-c(1:2)], all=TRUE)
...
LakLev_lvl <- lvl(x=Data_X$LakLev, variables=LakLev_vars, fitby="lakeid", lvl2Thresh=setThresh)
...
Zmix_lvl <- lvl(x=Data_X$Zmix, variables=Zmix_vars, fitby="lakeid", lvl2Thresh=setThresh)
...
LiExt_lvl <- lvl(x=Data_X$LiExt, variables=LiExt_vars, fitby="lakeid", lvl2Thresh=setThresh)
...
Phys_lvl <- lvl(x=Data_X$Phys, variables=Phys_vars, fitby="lakeid", lvl2Thresh=setThresh)
...
Ions_lvl <- lvl(x=Data_X$Ions, variables=Ions_vars, fitby="lakeid", lvl2Thresh=setThresh)
Ions <- merge(Ions_Params, Ions_lvl[complete.cases(Ions_lvl),], all=TRUE)


# ========
# = Chem =
# ========
# AllChem <- c("ph","alk","dic", "tic", "doc", "toc", "no3no2","nh4", "totnf", "totnuf", "totpf", "totpuf", "drsif", "brsif", "brsiuf", "tpm") #changed in _v6
...
Chem_lvl <- lvl(x=Data_X$Chem, variables=Chem_vars, fitby="lakeid", lvl2Thresh=setThresh)
Chem <- merge(Chem_Params, Chem_lvl[complete.cases(Chem_lvl),], all=TRUE)
...
Chl_lvl <- lvl(x=Data_X$Chl, variables=Chl_vars, fitby="lakeid", lvl2Thresh=setThresh)
...
# ========
dz0 <- Data_X$Zoop#_v8
Data_X$Zoop[Data_X$Zoop==0] <- NA #_v8: I think there were a lot of instances where missing values were present as 0's instead of NA's. I think I originally dealth with this issue in the few lines below, where I replaced extremes recorded as 0's with NA's.  However, now that I want to use the original series, I want the same treatment for the full data set.
...
Zoop_lvl <- lvl(x=Data_X$Zoop, variables=Zoop_vars, fitby="lakeid", lvl2Thresh=setThresh)
...
Fish_ByGear_lvl <- lvl(x=Data_X$Fish_ByGear[gearInd,], variables=Fish_ByGear_vars, fitby="lakeid", lvl2Thresh=setThresh)

AllFish_ByGear <- FishVars
# AllFish_ByGear <- c("total_caught","cpue1_Sum","Nfish", "SumWei", "cpue3_WeiEff", "cpue4_LengEff", "mean_Leng","mean_Wei", "max_Leng", "max_Wei", "min_Leng", "min_Wei")
Fish_ByGear2_Ext <- aggregate(Data_X$Fish_ByGear[,AllFish_ByGear], by=Data_X$Fish_ByGear[,c("gearid","year4", "lakeid")], FUN=max, na.rm=TRUE)
Fish_ByGear2_Ext <- Inf2NA(Fish_ByGear2_Ext)
Fish_ByGear2_Ext <- subset(Fish_ByGear2_Ext, gearid=="VGN") #I should eventually remove this once I can introduce gearid as a covariate

AllFish_ByGear2 <- FishVars
Fish_ByGear_fit2 <- calcGEV("Fish_ByGear2", fitBy="lakeid")
...
Fish_ByGear2_lvl <- lvl(x=Data_X$Fish_ByGear[gearInd2,], variables=Fish_ByGear2_vars, fitby="lakeid", lvl2Thresh=setThresh)
...
levels(Lake_Params3[,"Variable"]) <- c("CPUE", "Weight_PUE", "Zoop_Density", "Zoop_Mass", "Chla", "pH", "Alk", "DIC", "TIC", "DOC", "TOC", "NO3NO2", "NH4", "TotNuf", "TotPuf", "drSif", "BrSiuf", "TPM", "Cl", "SO4", "Ca", "Mg", "Na", "K", "Fe", "Mn", "Cond", "wTemp", "O2", "Secchi", "ExtCoef", "Zmix", "LakeLevel")
...
All_Params[,"Level1_normTime"] <- normTime(All_Params)
All_Params[,"Level2_normTime"] <- normTime(All_Params, Level=2)
All_Params[,"Level1_logNormTime"] <- lnormTime(All_Params)
All_Params[,"Level2_logNormTime"] <- lnormTime(All_Params, Level=2)


sAP <- All_Params[,c("Type", "fitBy", "Variable", "Duration", "N", "mu_0", "sig_0", "sh_0", "Level1", "Level2", "mean", "sd", "Level1_time", "Level2_time", "Level1_normTime", "Level2_normTime", "Level1_logNormTime", "Level2_logNormTime")]
# sAP[is.nan(sAP)] <- NA
# sAP <- Inf2NA(sAP)
sAP <- sAP[order(as.character(sAP[,"Variable"])),]
# plot(sAP[,"sh_0"], log10(sAP[,"Level2_time"])) #as shape increases, the return time of 1.2* maximum decreases
# plot(log10(sAP[,"Level1_normTime"]), log10(sAP[,"Level1_time"])); abline(a=0, b=1) #for the mean+2*sd return level, the normal and GEV distributions give similar return times
# plot(log10(sAP[,"Level2_normTime"]), log10(sAP[,"Level2_time"])); abline(a=0, b=1) #for the 1.2*max return level, there is little correlation between the return times given by the normal and GEV distributions, but the GEV tends to predict much shorter return times.

# save(All_Params, Lake_Params3, sAP, file="All_Params_TurnExtreme_Fat_Data_v7.RData")
save.image(file="All_Params_TurnExtreme_Fat_Data_v8.RData")