...
not4tree0 <- c("a1", "a2", "a3", "AICc", "b1", "b2", "b3", "Community", "Duration", "level", "Level2_logNormTime", "Level2_normTime", "Level2_time", "logMean", "logSd", "mean", "mu_0", "nll", "Period", "sd", "shape.sig", "sig_0", "Species", "taxID", "taxLvl", "variable")
...
data.tree00 <- data.2[,names(data.2)[!names(data.2)%in%c(not4tree.full)]]
...
dt.party.imp <- sort(varimp(dt.party), decreasing=TRUE)
